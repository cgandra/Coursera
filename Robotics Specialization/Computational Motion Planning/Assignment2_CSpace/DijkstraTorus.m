...
%     
    ...
    % *******************************************************************
    neighbors=[sub2ind(size(map), mod(i, nrows)+1, j);
        sub2ind(size(map), mod(i-2, nrows)+1, j); 
        sub2ind(size(map), i, mod(j, ncols)+1);
        sub2ind(size(map), i, mod(j-2, ncols)+1)];
    
    for n=1:length(neighbors)
        [ii, jj] = ind2sub([nrows,ncols], neighbors(n));
