...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    err_s = K_gain @ (y_k - p_check)

    # 3.3 Correct predicted state
    p_check += err_s[:3]
    v_check += err_s[3:6]
    q_check = Quaternion(axis_angle=err_s[6:9]).quat_mult_left(Quaternion(*q_check))
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    c_ns = q_prev.to_mat()
    q_cur = Quaternion(axis_angle=imu_w.data[k-1]*delta_t)
    f_ns = (c_ns @ imu_f.data[k-1] + g)
    p_est[k] = p_est[k-1] + delta_t*v_est[k-1] + 0.5*(delta_t**2)*f_ns
    v_est[k] = v_est[k-1] + delta_t*f_ns
    q_est[k] = q_prev.quat_mult_left(q_cur)

    # 1.1 Linearize the motion model and compute Jacobians

    F = np.eye(9)
    F[0:3, 3:6] = delta_t*np.eye(3)
    F[3:6, 6:9] = -skew_symmetric(c_ns @ imu_f.data[k - 1]) * delta_t

    # 2. Propagate uncertainty
    Q = np.eye(6) # IMU noise covariance
    ...
    if imu_f.t[k] in gnss_t:
        ...
        p_est[k], v_est[k], q_est[k], p_cov[k] = \
            measurement_update(var_gnss, p_cov[k], gnss.data[gnss_i], p_est[k], v_est[k], q_est[k])
    if imu_f.t[k] in lidar_t:
        lidar_i = lidar_t.index(imu_f.t[k])
        p_est[k], v_est[k], q_est[k], p_cov[k] = \
            measurement_update(var_lidar, p_cov[k], lidar.data[lidar_i], p_est[k], v_est[k], q_est[k])
