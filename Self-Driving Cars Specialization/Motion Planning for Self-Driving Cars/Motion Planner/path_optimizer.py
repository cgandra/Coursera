...
class PathOptimizer:
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        d1 = np.array(d)
        s1 = np.array(s)
        thetas = a1*s1 + (b1/2)*(s1**2) + (c1/3)*(s1**3) + (d1/4)*(s1**4)
        return thetas
        # ------------------------------------------------------------------
    ...
    def sample_spiral(self, p):
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        t_points = list(self.thetaf(a, b, c, d, s_points))
        x_points = list(scipy.integrate.cumtrapz(np.cos(t_points),s_points))
        y_points = list(scipy.integrate.cumtrapz(np.sin(t_points),s_points))
